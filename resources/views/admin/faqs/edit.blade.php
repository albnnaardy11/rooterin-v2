...
<div class="space-y-12" x-data="{ 
    ...
    placement: '{{ old('placement', $faq->placement) }}'
}">
    <div class="flex items-center gap-6">
        ...
        <div>
            <h1 class="text-3xl font-heading font-black text-white tracking-tight italic">Intelligence <span class="text-primary not-italic">Update.</span></h1>
            <p class="text-slate-500 font-medium uppercase text-[10px] tracking-[0.3em]">Modifying Logic Entry : ID/{{ str_pad($faq->id, 3, '0', STR_PAD_LEFT) }}</p>
        </div>
    </div>

    <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-12">
        @csrf
        @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative group">
                        ...
                        <textarea name="answer" x-model="answer" required rows="8" placeholder="Enter the detailed logical response..." 
                                  class="w-full bg-slate-950 border border-white/5 rounded-[2.5rem] pl-16 pr-8 py-6 text-white placeholder-slate-700 focus:outline-none focus:border-primary/50 transition-all font-medium text-base shadow-inner leading-relaxed">{{ old('answer', $faq->answer) }}</textarea>
                    </div>
        ...
        <div class="lg:col-span-4 space-y-8">
            <div class="bg-slate-900/50 p-10 rounded-[3rem] border border-white/5 space-y-10 sticky top-12 shadow-2xl shadow-black/40">
                <div class="space-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="cursor-pointer group">
                            <input type="radio" name="is_active" value="1" class="hidden" {{ old('is_active', $faq->is_active) ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer group">
                            <input type="radio" name="is_active" value="0" class="hidden" {{ !old('is_active', $faq->is_active) ? 'checked' : '' }}>
                ...
                <div class="pt-10 border-t border-white/5">
                    ...
                    <p class="text-center text-[8px] font-black text-slate-600 uppercase tracking-widest mt-6">Protocol Version 2.0.4-L</p>
                </div>
            </div>
        </div>
    </form>
