...
<div class="space-y-12">
    <div class="flex items-center gap-6">
        ...
        <div>
            <h1 class="text-3xl font-heading font-black text-white tracking-tight">Edit <span class="text-primary italic">Category.</span></h1>
            <p class="text-slate-500 font-medium uppercase text-[10px] tracking-[0.3em]">Update FAQ group: {{ $faq_category->name }}</p>
        </div>
    </div>

    <form action="{{ route('admin.faq-categories.update', $faq_category->id) }}" method="POST" class="max-w-2xl">
        @csrf
        @method('PUT')
        <div class="bg-slate-900/50 p-8 sm:p-12 rounded-[3rem] border border-white/5 backdrop-blur-xl space-y-8">
            <div>
                ...
                <input type="text" name="name" required value="{{ old('name', $faq_category->name) }}" placeholder="e.g. Teknis & Layanan" 
            ...
            <div>
                ...
                <input type="text" name="icon" required value="{{ old('icon', $faq_category->icon) }}" placeholder="e.g. ri-tools-fill" 
                       class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder-slate-600 focus:outline-none focus:border-primary/50 transition-all font-bold">
            ...
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $faq_category->order) }}" 
                       class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder-slate-600 focus:outline-none focus:border-primary/50 transition-all font-bold">
